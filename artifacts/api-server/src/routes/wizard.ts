...
router.put("/wizard/progress", async (req: Request, res: Response) => {
  try {
    ...
    // Only overwrite stepData when the caller actually sent a draft payload;
    // a plain "advance to next step" PUT (currentStep only) must not blow
    // away the previous step's already-saved draft.
