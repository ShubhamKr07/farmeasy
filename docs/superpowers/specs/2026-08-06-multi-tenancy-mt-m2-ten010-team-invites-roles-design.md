...
- **The JWT `user_role` claim is stale and points at the wrong axis — and it has LIVE authorization consumers, not just cosmetic ones.** `custom_access_token_hook` (`supabase/migrations/00001_...`) injects `public.users.role` — the **deprecated** `user_role` enum (`technician | supervisor | quality_lead | facility_lead`, an operational/seniority axis superseded by `organization_members.role` per ADR-005, not yet dropped). Readers: mobile's `useUserRole()` (`artifacts/farmeasy/hooks/useUserRole.ts`) and mobile `cycles.tsx` (a UI affordance on `isSupervisor`); **and, critically, `routes/cycles.ts` on the backend** — `extractRole(req)` reads the claim and `isSupervisorOrLead(role)` gates real authorization: viewing cycle **history** (`GET /cycles?status=history`) and **editing completed cycles** are restricted to `supervisor`/`facility_lead`. Repointing the claim to org roles therefore cannot be done in isolation — it forces a mapping of these operational gates onto the new axis (see Design decision below). No users are actually `supervisor`/`facility_lead` today (all default `technician`), so these gates currently block everyone.
...
**Operational-role gates collapse to `role !== 'technician'`.** The existing `cycles.ts` / mobile `cycles.tsx` gates that were `isSupervisorOrLead` (supervisor/facility_lead) become "any non-technician member" (owner or admin) — technicians remain restricted from cycle-history view and completed-cycle edits, owner/admin gain them. This is the sensible collapse of the operational axis onto the access axis (technician is the restricted, mobile-only role; owner/admin are privileged). It is a deliberate, small behavioral change (today these gates block everyone since no real supervisors exist); TEN-010 owns updating `routes/cycles.ts`'s `extractRole`/`isSupervisorOrLead` and mobile `cycles.tsx` to the new check.
...
- `routes/cycles.ts` — `extractRole`/`isSupervisorOrLead` updated to read the org role (from `req.tenant.role`, now that the resolver carries it) and gate on `role !== 'technician'`; the three call sites (history view, two completed-cycle edits) unchanged in intent.
...
- `hooks/useUserRole.ts` — repoint to the org-role claim; `isSupervisor` becomes "non-technician." `lib/api-zod/src/roles.ts`'s `UserRole` type + labels + `isSupervisorOrLead` updated (or replaced) for the org axis.
- `app/(tabs)/cycles.tsx` — the `isSupervisor` UI gate follows the same `role !== 'technician'` mapping.
...
- **The JWT-hook repoint is the highest-risk single piece** — it changes the claim every authenticated client reads, AND it changes the value domain that live authorization code depends on. `routes/cycles.ts` (backend, history + completed-edit gates) and mobile `cycles.tsx`/`useUserRole` all read the claim; the repoint must land together with their `role !== 'technician'` rewrite or cycle-history/completed-edits break. Needs the same empirical, real-`app.ts`, real-JWT verification discipline TEN-008/Task 12.5 established, plus a full audit that no reader still expects the operational-role values (`supervisor | quality_lead | facility_lead`). Consider reading the org role from `req.tenant.role` server-side in `cycles.ts` (already resolved, more reliable than the JWT claim) rather than from the claim.
