...
**Status:** Accepted — cutover verified 2026-07-26. All 24 operational tables
row-count-matched Neon↔Supabase (Task 8), pgvector embeddings intact (85/85
at 1536 dims), farmsmart-api and farmsmart-recommender redeployed against
Supabase and healthz-verified (Task 9). Neon retained as rollback until the
7-day retention window closes (Task 10).
**Date:** 2026-07-26
...
> **Note:** The "single project, no staging/prod split" portion of the
> sentence above is **superseded by ADR-004**, which introduces a separate
> staging project and a staging/prod split. The Supabase-provider decision
> itself remains Accepted and in force.
